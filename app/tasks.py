...
def classify_images(self, keywords=[]):
    ...
    with app.app_context():
        ...
        dataset = Dataset(logging=True, extractors=extractors)
        samples = dataset._read_samples(root=temp_folder, no_sub_directories=True)
        dataset.data = []
        ...
        for sample in samples:
            dataset.data.append(dataset._feature_vector(data))
        dataset.normalize()
        dataset.save(dataset_file)
        update_progress(self, 75)
        # step 4 - use trained classifier to predict class
        # todo link to right classifier
        classifier = joblib.load(model_file)
        results = classifier.predict(dataset.data)
        # step 5 - return list of url and prediced category pairs
        # todo link uris to right labels
        labels = ['sign', 'chart', 'architecture', 'drawn', 'painting', 'social', 'portrait', 'screenshot', 'flag', 'vehicles', 'logo', 'document', 'object', 'scheme', 'map', 'icon', 'landscape', 'scenery']
